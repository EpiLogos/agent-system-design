...
fn validate_value(value: Value) -> Vec<String> {
    let manifest = serde_json::from_value(value).expect("fixture must retain manifest shape");
...
fn invalid_promotion_fixture_is_rejected() {
    ...
    assert!(errors
        .iter()
        .any(|error| error.contains("silent authority promotion")));
}
...
#[test]
fn schema_surface_matches_manifest_version() {

...
const root = process.env.FACTORY_INTEROP_ROOT || path.resolve(__dirname, '../..');
...
const kindRef = (kind, value) => typeof value === 'string' && new RegExp(`^${kind}:[0-7][0-9A-HJKMNP-TV-Z]{25}$`).test(value);
const subjectKey = (s) => `${s.subjectRef}|${s.subjectRevision}|${s.stateRef}`;

function checkOwners(node, where = '#') {
  if (!node || typeof node !== 'object' || Array.isArray(node)) return;
  if (node.properties) {
    for (const [name, child] of Object.entries(node.properties)) {
      assert(typeof child['x-semantic-owner'] === 'string' && child['x-semantic-owner'].trim(), `${where}/${name} missing one semantic owner`);
      checkOwners(child, `${where}/${name}`);
    }
  }
  if (node.$defs) for (const [name, child] of Object.entries(node.$defs)) checkOwners(child, `${where}/$defs/${name}`);
  if (node.items && typeof node.items === 'object') checkOwners(node.items, `${where}/items`);
  if (node.anyOf) for (const [i, child] of node.anyOf.entries()) checkOwners(child, `${where}/anyOf/${i}`);
}

function wholeHasUnmetObligations(v) {
  const whole = v.operativeWhole || {};
  const required = new Set(whole.requiredObligations || []);
  const satisfied = new Set(whole.satisfiedObligations || []);
  return [...required].some((obligation) => !satisfied.has(obligation));
}

function rejectsAnti(item) {
  const v = item.value || {};
  switch (item.id) {
...
const schemaSet = load('contracts/factory/interop/schema-set.json');
const fixtureSet = load('contracts/factory/fixtures/interop/fixture-set.json');
...
const schemas = new Map(schemaSet.schemas.map((entry) => [entry.instanceKey, entry]));
const sections = new Map(fixtureSet.sections.map((entry) => [entry.instanceKey, entry]));
...
for (const [key, entry] of schemas) {
  const schema = load(entry.path), fixture = load(sections.get(key).path);
...
assert(run.projectionProvenance.length >= 2 && new Set(run.projectionProvenance.map((p) => p.providerRef)).size >= 2 && new Set(run.projectionProvenance.map((p) => p.externalId)).size >= 2, 'projection change not exercised');
assert(run.projectionProvenance.every((p) => p.canonicalRef === identity.projectBinding.projectRef && p.projectedRevision === run.runEnvelope.runRevision), 'projection identity/revision drift');
...
const cx = context.contextResolution, available = new Set(cx.availableRefs), retrieved = new Set(cx.retrievedRefs), loaded = new Set(cx.loadedRefs);
...
[evidence.evidenceEnvelope.subjectState, evidence.assessmentEnvelope.subjectState, closure.closureEnvelope.subjectState, closure.gateDecisionEnvelope.subjectState].forEach((s) => assert(subjectKey(s) === current, 'verification subject-state mismatch'));
...
assert(demand.executionDemand.requiredAffordances.every((a) => offer.workcellOffer.affordances.includes(a)), 'required affordance unavailable');
...
const anti = load(fixtureSet.antiFixturesPath), expectedAnti = new Set(['action-as-capability-identity-collapse','binding-as-ref','model-as-agent','session-as-agent','provider-as-project','stale-subject-state-evidence','plausible-artifact-partial-evidence-as-full-closure','representative-evidence-without-coverage-contract']);
assert(anti.fixtureVersion === 'factory.interop-fixtures/v1' && anti.antiFixtures.length === expectedAnti.size && anti.antiFixtures.every((item) => expectedAnti.has(item.id)), 'anti-fixture set incomplete');
anti.antiFixtures.forEach((item) => assert(item.mustReject === true && rejectsAnti(item), `anti fixture unexpectedly accepted: ${item.id}`));
console.log(`Node CR-001 interop PASS (${schemas.size} schema sections, ${anti.antiFixtures.length} anti-fixtures)`);

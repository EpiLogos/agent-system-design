function residue(kind, position, value) {
  return { kind, position, value, provenance: { fixture: 'native-baseline' } };
}

export function createNativeBaselinePolicy() {
  return {
    nextAct({ circuit }) {
    ...
    interpret({ circuit, difference }) {
      ...
      if (from === 'P1') return { destination: 'P4', rationale: 'Capability evidence returned.', residueDelta: { create: [residue('material', 'P1', difference)] } };
    ...
    createReentryDelta() {
      return { achieved_artifact_refs: ['artifact:native:matched-runtime-baseline'], changed_assumptions: [], opened_questions: [], provenance: { fixture: 'native-baseline', host: 'native' } };
    }
  };
}
